...
2. Затем запускается `python manage.py ensure_admin` (создает/обновляет суперпользователя по env)
...
5. После этого стартует Gunicorn
...
После успешного `finalize` ML сервис автоматически пересчитывает snapshot-прогнозы
на следующий год и сохраняет их в таблицу `ml_prediction_snapshots`.
...
GET  /api/snapshots/ — предрасчитанные прогнозы из БД (year обязателен)
POST /api/snapshots/rebuild/ — принудительный пересчет snapshot-прогнозов
```
